  ...
  const fetchProducts = async (filters: number[] = []) => {
    try {
      const res =
        filters.length > 0
          ? await axios.post(
              '/api/products/by-category/36/filtered',
              { value_ids: filters },
              { withCredentials: true }
            )
          : await axios.get('/api/products/category/36', { withCredentials: true });
...
export default OtherSmallAnimalsPage;
