  ...
  const fetchProducts = async (filters: number[] = []) => {
    try {
      const res =
        filters.length > 0
          ? await axios.post(
              '/api/products/by-category/23/filtered',
              { value_ids: filters },
              { withCredentials: true }
            )
          : await axios.get('/api/products/category/23', { withCredentials: true });
...
export default CatLitterBoxesPage;
