...
const CatScratchingFurniturePage = () => {
  ...
  const fetchProducts = async (filters: number[] = []) => {
    try {
      const res =
        filters.length > 0
          ? await axios.post(
              '/api/products/by-category/28/filtered',
              { value_ids: filters },
              { withCredentials: true }
            )
          : await axios.get('/api/products/category/28', { withCredentials: true });
...
export default CatScratchingFurniturePage;
